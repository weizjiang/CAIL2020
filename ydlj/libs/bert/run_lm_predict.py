...
def convert_examples_to_features(examples, max_seq_length, tokenizer, max_predictions_per_seq):
  ...
  for (ex_index, example) in enumerate(examples):
    ...
    features, tokens = convert_single_example(ex_index, example,
                                     max_seq_length, tokenizer, max_predictions_per_seq)
    all_features.extend(features)
    all_tokens.extend(tokens)

  return all_features, all_tokens

def create_masked_lm_prediction(input_ids, mask_position, mask_count=1, tokenizer=None):
  MASKED_TOKEN = "[MASK]"
...
def convert_single_example(ex_index, example, max_seq_length,
                           tokenizer, max_predictions_per_seq):
...
def create_sequential_mask(input_tokens, input_ids, input_mask, segment_ids,
  ...
  while i < len(input_tokens) - 1:
    mask_count = 1
    while is_subtoken(input_tokens[i+mask_count]) and mask_count < max_predictions_per_seq:
      mask_count += 1

    input_ids_new, masked_lm_positions, masked_lm_labels = create_masked_lm_prediction(input_ids, i, mask_count, tokenizer)
...
def parse_result(result, all_tokens, output_file=None):
  with tf.gfile.GFile(output_file, "w") as writer:
    ...
    for word_loss in result:
      ...
      while is_subtoken(all_tokens[i]) and token_count_per_word < len(word_loss) - 1:
      ...
      if all_tokens[i] == "[SEP]":
        ...
        sentence["nce"] = -sentence_loss / word_count_per_sent
...
def main(_):
  ...
  features, all_tokens = convert_examples_to_features(predict_examples,
                                          FLAGS.max_seq_length, tokenizer, FLAGS.max_predictions_per_seq)
